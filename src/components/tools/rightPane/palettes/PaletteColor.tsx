...
export const NormalColorBtn = ({
  ...
  return (
    <button
      type="button"
      className={`relative w-full aspect-square grid place-items-center rounded-full shadow-md transition-opacity duration-200 ease-in-out hover:opacity-90`}
...
export const AddColorBtn = ({ handleClick }: { handleClick: () => void }) => (
  <button
    type="button"
    className="w-full aspect-square grid place-items-center rounded-full shadow-md active:shadow-inner"
    onClick={handleClick}
  >
    <IconPlus size={20} />
  </button>
