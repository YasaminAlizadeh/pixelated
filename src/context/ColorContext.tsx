...
export type ColorContextType = {
  ...
  addToPalette: (id: string, color: ColorResult) => void;
  removeFromPalette: (id: string, color: ColorResult) => void;
...
const ColorProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const addToPalette = useCallback(
    (id: string, color: ColorResult) => {
      const selectedPaletteIndex = palettes.findIndex((pal) => pal.id === id);

      if (selectedPaletteIndex === -1) return;

      const selectedPalette = palettes[selectedPaletteIndex];

      if (selectedPalette.colors.find((clr) => clr.hex === color.hex)) return;

      const palettesCopy = [...palettes];

      palettesCopy.splice(selectedPaletteIndex, 1, {
        ...selectedPalette,
  ...
  const removeFromPalette = useCallback(
    (id: string, color: ColorResult) => {
      ...
      palettesCopy.splice(selectedPaletteIndex, 1, {
        ...selectedPalette,
        colors: selectedPalette.colors.filter((clr) => clr.hex !== color.hex),
      });
